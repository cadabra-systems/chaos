...
#ifndef Chaos_Filesystem_hpp
#define Chaos_Filesystem_hpp

#ifndef CHAOS_OS_DETECTED
#if defined(__APPLE__) && defined(__MACH__)
#define CHAOS_OS_MACOS
#elif defined(__linux__)
#define CHAOS_OS_LINUX
#if defined(__ANDROID__)
#define CHAOS_OS_ANDROID
#endif
#elif defined(_WIN64)
#define CHAOS_OS_WINDOWS
#define CHAOS_OS_WIN64
#elif defined(_WIN32)
#define CHAOS_OS_WINDOWS
#define CHAOS_OS_WIN32
#else
#error "Operating system currently not supported!"
#endif
#define CHAOS_OS_DETECTED
#endif

#if defined(CHAOS_FILESYSTEM_IMPLEMENTATION)
#define CHAOS_EXPAND_IMPL
#define CHAOS_INLINE
#ifdef CHAOS_OS_WINDOWS
#define CHAOS_FS_API
#define CHAOS_FS_API_CLASS
#else
#define CHAOS_FS_API __attribute__((visibility("default")))
#define CHAOS_FS_API_CLASS __attribute__((visibility("default")))
#endif
#elif defined(CHAOS_FILESYSTEM_FWD)
#define CHAOS_INLINE
#ifdef CHAOS_OS_WINDOWS
#define CHAOS_FS_API extern
#define CHAOS_FS_API_CLASS
#else
#define CHAOS_FS_API extern
#define CHAOS_FS_API_CLASS
...
#else
#define CHAOS_EXPAND_IMPL
#define CHAOS_INLINE inline
#define CHAOS_FS_API
#define CHAOS_FS_API_CLASS
#endif

#ifdef CHAOS_EXPAND_IMPL
...
#include <sys/types.h>
#include <unistd.h>
#ifdef CHAOS_OS_ANDROID
#include <android/api-level.h>
#endif
#endif
#ifdef CHAOS_OS_MACOS
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// UTF8-Everywhere is the original behaviour of chaos::filesystem. With this define you can
// enable the more standard conforming implementation option that uses wstring on Windows
// as chaos::filesystem::string_type.
// #define CHAOS_WIN_WSTRING_STRING_TYPE
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// #define CHAOS_RAISE_UNICODE_ERRORS
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

// chaos::filesystem version in decimal (major * 10000 + minor * 100 + patch)
#define CHAOS_FILESYSTEM_VERSION 10210L

namespace chaos { namespace filesystem {

// temporary existing exception type for yet unimplemented parts
class CHAOS_FS_API_CLASS not_implemented_exception : public std::logic_error
{
public:
...
// 30.10.8 class path
class CHAOS_FS_API_CLASS path
#if defined(CHAOS_OS_WINDOWS) && defined(CHAOS_WIN_WSTRING_STRING_TYPE)
#define CHAOS_USE_WCHAR_T
...
public:
	...
	enum format {
		generic_format,  ///< The generic format, internally used by
						 ///< chaos::filesystem::path with slashes
...
CHAOS_FS_API void swap(path& lhs, path& rhs) noexcept;
CHAOS_FS_API size_t hash_value(const path& p) noexcept;
CHAOS_FS_API bool operator==(const path& lhs, const path& rhs) noexcept;
CHAOS_FS_API bool operator!=(const path& lhs, const path& rhs) noexcept;
CHAOS_FS_API bool operator<(const path& lhs, const path& rhs) noexcept;
CHAOS_FS_API bool operator<=(const path& lhs, const path& rhs) noexcept;
CHAOS_FS_API bool operator>(const path& lhs, const path& rhs) noexcept;
...
CHAOS_FS_API path operator/(const path& lhs, const path& rhs);
...
class CHAOS_FS_API_CLASS filesystem_error : public std::system_error
{
public:
...
class CHAOS_FS_API_CLASS path::iterator
{
public:
...
class CHAOS_FS_API_CLASS file_status
{
public:
...
class CHAOS_FS_API_CLASS directory_entry
{
public:
...
class CHAOS_FS_API_CLASS directory_iterator
{
public:
	class CHAOS_FS_API_CLASS proxy
	{
	public:
...
CHAOS_FS_API directory_iterator begin(directory_iterator iter) noexcept;
CHAOS_FS_API directory_iterator end(const directory_iterator&) noexcept;

// 30.10.14 class recursive_directory_iterator
class CHAOS_FS_API_CLASS recursive_directory_iterator
{
public:
	using iterator_category = std::input_iterator_tag;
	using value_type = directory_entry;
...
CHAOS_FS_API recursive_directory_iterator begin(recursive_directory_iterator iter) noexcept;
CHAOS_FS_API recursive_directory_iterator end(const recursive_directory_iterator&) noexcept;

// 30.10.15 filesystem operations
CHAOS_FS_API path absolute(const path& p);
CHAOS_FS_API path absolute(const path& p, std::error_code& ec);

CHAOS_FS_API path canonical(const path& p);
CHAOS_FS_API path canonical(const path& p, std::error_code& ec);

CHAOS_FS_API void copy(const path& from, const path& to);
CHAOS_FS_API void copy(const path& from, const path& to, std::error_code& ec) noexcept;
CHAOS_FS_API void copy(const path& from, const path& to, copy_options options);
CHAOS_FS_API void copy(const path& from, const path& to, copy_options options, std::error_code& ec) noexcept;

CHAOS_FS_API bool copy_file(const path& from, const path& to);
CHAOS_FS_API bool copy_file(const path& from, const path& to, std::error_code& ec) noexcept;
CHAOS_FS_API bool copy_file(const path& from, const path& to, copy_options option);
CHAOS_FS_API bool copy_file(const path& from, const path& to, copy_options option, std::error_code& ec) noexcept;

CHAOS_FS_API void copy_symlink(const path& existing_symlink, const path& new_symlink);
CHAOS_FS_API void copy_symlink(const path& existing_symlink, const path& new_symlink, std::error_code& ec) noexcept;

CHAOS_FS_API bool create_directories(const path& p);
CHAOS_FS_API bool create_directories(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool create_directory(const path& p);
CHAOS_FS_API bool create_directory(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool create_directory(const path& p, const path& attributes);
CHAOS_FS_API bool create_directory(const path& p, const path& attributes, std::error_code& ec) noexcept;

CHAOS_FS_API void create_directory_symlink(const path& to, const path& new_symlink);
CHAOS_FS_API void create_directory_symlink(const path& to, const path& new_symlink, std::error_code& ec) noexcept;

CHAOS_FS_API void create_hard_link(const path& to, const path& new_hard_link);
CHAOS_FS_API void create_hard_link(const path& to, const path& new_hard_link, std::error_code& ec) noexcept;

CHAOS_FS_API void create_symlink(const path& to, const path& new_symlink);
CHAOS_FS_API void create_symlink(const path& to, const path& new_symlink, std::error_code& ec) noexcept;

CHAOS_FS_API path current_path();
CHAOS_FS_API path current_path(std::error_code& ec);
CHAOS_FS_API void current_path(const path& p);
CHAOS_FS_API void current_path(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool exists(file_status s) noexcept;
CHAOS_FS_API bool exists(const path& p);
CHAOS_FS_API bool exists(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool equivalent(const path& p1, const path& p2);
CHAOS_FS_API bool equivalent(const path& p1, const path& p2, std::error_code& ec) noexcept;

CHAOS_FS_API uintmax_t file_size(const path& p);
CHAOS_FS_API uintmax_t file_size(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API uintmax_t hard_link_count(const path& p);
CHAOS_FS_API uintmax_t hard_link_count(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool is_block_file(file_status s) noexcept;
CHAOS_FS_API bool is_block_file(const path& p);
CHAOS_FS_API bool is_block_file(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_character_file(file_status s) noexcept;
CHAOS_FS_API bool is_character_file(const path& p);
CHAOS_FS_API bool is_character_file(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_directory(file_status s) noexcept;
CHAOS_FS_API bool is_directory(const path& p);
CHAOS_FS_API bool is_directory(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_empty(const path& p);
CHAOS_FS_API bool is_empty(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_fifo(file_status s) noexcept;
CHAOS_FS_API bool is_fifo(const path& p);
CHAOS_FS_API bool is_fifo(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_other(file_status s) noexcept;
CHAOS_FS_API bool is_other(const path& p);
CHAOS_FS_API bool is_other(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_regular_file(file_status s) noexcept;
CHAOS_FS_API bool is_regular_file(const path& p);
CHAOS_FS_API bool is_regular_file(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_socket(file_status s) noexcept;
CHAOS_FS_API bool is_socket(const path& p);
CHAOS_FS_API bool is_socket(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API bool is_symlink(file_status s) noexcept;
CHAOS_FS_API bool is_symlink(const path& p);
CHAOS_FS_API bool is_symlink(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API file_time_type last_write_time(const path& p);
CHAOS_FS_API file_time_type last_write_time(const path& p, std::error_code& ec) noexcept;
CHAOS_FS_API void last_write_time(const path& p, file_time_type new_time);
CHAOS_FS_API void last_write_time(const path& p, file_time_type new_time, std::error_code& ec) noexcept;

CHAOS_FS_API void permissions(const path& p, perms prms, perm_options opts = perm_options::replace);
CHAOS_FS_API void permissions(const path& p, perms prms, std::error_code& ec) noexcept;
CHAOS_FS_API void permissions(const path& p, perms prms, perm_options opts, std::error_code& ec);

CHAOS_FS_API path proximate(const path& p, std::error_code& ec);
CHAOS_FS_API path proximate(const path& p, const path& base = current_path());
CHAOS_FS_API path proximate(const path& p, const path& base, std::error_code& ec);

CHAOS_FS_API path read_symlink(const path& p);
CHAOS_FS_API path read_symlink(const path& p, std::error_code& ec);

CHAOS_FS_API path relative(const path& p, std::error_code& ec);
CHAOS_FS_API path relative(const path& p, const path& base = current_path());
CHAOS_FS_API path relative(const path& p, const path& base, std::error_code& ec);

CHAOS_FS_API bool remove(const path& p);
CHAOS_FS_API bool remove(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API uintmax_t remove_all(const path& p);
CHAOS_FS_API uintmax_t remove_all(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API void rename(const path& from, const path& to);
CHAOS_FS_API void rename(const path& from, const path& to, std::error_code& ec) noexcept;

CHAOS_FS_API void resize_file(const path& p, uintmax_t size);
CHAOS_FS_API void resize_file(const path& p, uintmax_t size, std::error_code& ec) noexcept;

CHAOS_FS_API space_info space(const path& p);
CHAOS_FS_API space_info space(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API file_status status(const path& p);
CHAOS_FS_API file_status status(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API bool status_known(file_status s) noexcept;

CHAOS_FS_API file_status symlink_status(const path& p);
CHAOS_FS_API file_status symlink_status(const path& p, std::error_code& ec) noexcept;

CHAOS_FS_API path temp_directory_path();
CHAOS_FS_API path temp_directory_path(std::error_code& ec) noexcept;

CHAOS_FS_API path weakly_canonical(const path& p);
CHAOS_FS_API path weakly_canonical(const path& p, std::error_code& ec) noexcept;
...
public:
	...
	{
#if defined(CHAOS_OS_WINDOWS) && !defined(__GNUC__)
...
class CHAOS_FS_API_CLASS u8arguments
{
public:
...
namespace detail {
// CHAOS_FS_API void postprocess_path_with_format(path::impl_string_type& p, path::format fmt);
enum utf8_states_t { S_STRT = 0, S_RJCT = 8 };
CHAOS_FS_API void appendUTF8(std::string& str, uint32_t unicode);
CHAOS_FS_API bool is_surrogate(uint32_t c);
CHAOS_FS_API bool is_high_surrogate(uint32_t c);
CHAOS_FS_API bool is_low_surrogate(uint32_t c);
CHAOS_FS_API unsigned consumeUtf8Fragment(const unsigned state, const uint8_t fragment, uint32_t& codepoint);
...
CHAOS_FS_API std::error_code make_error_code(portable_error err);
#ifdef CHAOS_OS_WINDOWS
CHAOS_FS_API std::error_code make_system_error(uint32_t err = 0);
#else
CHAOS_FS_API std::error_code make_system_error(int err = 0);
#endif
}  // namespace detail
...
CHAOS_INLINE std::error_code make_error_code(portable_error err)
{
#ifdef CHAOS_OS_WINDOWS
	switch (err) {
...
#ifdef CHAOS_OS_WINDOWS
CHAOS_INLINE std::error_code make_system_error(uint32_t err)
{
	return std::error_code(err ? static_cast<int>(err) : static_cast<int>(::GetLastError()), std::system_category());
}
#else
CHAOS_INLINE std::error_code make_system_error(int err)
{
...
CHAOS_INLINE bool in_range(uint32_t c, uint32_t lo, uint32_t hi)
{
	return (static_cast<uint32_t>(c - lo) < (hi - lo + 1));
}

CHAOS_INLINE bool is_surrogate(uint32_t c)
{
	return in_range(c, 0xd800, 0xdfff);
}

CHAOS_INLINE bool is_high_surrogate(uint32_t c)
{
	return (c & 0xfffffc00) == 0xd800;
}

CHAOS_INLINE bool is_low_surrogate(uint32_t c)
{
	return (c & 0xfffffc00) == 0xdc00;
}

CHAOS_INLINE void appendUTF8(std::string& str, uint32_t unicode)
{
	if (unicode <= 0x7f) {
...
#ifdef CHAOS_RAISE_UNICODE_ERRORS
...
CHAOS_INLINE unsigned consumeUtf8Fragment(const unsigned state, const uint8_t fragment, uint32_t& codepoint)
{
...
CHAOS_INLINE bool validUtf8(const std::string& utf8String)
{
...
CHAOS_INLINE bool startsWith(const std::string& what, const std::string& with)
{
...
CHAOS_INLINE void path::postprocess_path_with_format(path::impl_string_type& p, path::format fmt)
{
#ifdef CHAOS_RAISE_UNICODE_ERRORS
...
CHAOS_INLINE bool equals_simple_insensitive(const char* str1, const char* str2)
{
#ifdef CHAOS_OS_WINDOWS
#ifdef __GNUC__
...
CHAOS_INLINE const char* strerror_adapter(char* gnu, char*)
{
	return gnu;
}

CHAOS_INLINE const char* strerror_adapter(int posix, char* buffer)
{
	if(posix) {
...
CHAOS_INLINE std::string systemErrorText(ErrorNumber code = 0)
{
#if defined(CHAOS_OS_WINDOWS)
	LPVOID msgBuf;
...
CHAOS_INLINE void create_symlink(const path& target_name, const path& new_symlink, bool to_directory, std::error_code& ec)
{
	std::error_code tec;
...
CHAOS_INLINE void create_hardlink(const path& target_name, const path& new_hardlink, std::error_code& ec)
{
#if defined(__GNUC__) && __GNUC__ >= 8
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wcast-function-type"
...
#else
CHAOS_INLINE void create_symlink(const path& target_name, const path& new_symlink, bool, std::error_code& ec)
{
...
template <typename T>
CHAOS_INLINE file_status file_status_from_st_mode(T mode)
{
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE path resolveSymlink(const path& p, std::error_code& ec)
{
#ifdef CHAOS_OS_WINDOWS
...
#ifdef CHAOS_OS_WINDOWS
CHAOS_INLINE time_t timeFromFILETIME(const FILETIME& ft)
{
	ULARGE_INTEGER ull;
...
CHAOS_INLINE void timeToFILETIME(time_t t, FILETIME& ft)
{
	LONGLONG ll;
...
CHAOS_INLINE uintmax_t hard_links_from_INFO(const INFO* info)
{
	return static_cast<uintmax_t>(-1);
}

template <>
CHAOS_INLINE uintmax_t hard_links_from_INFO<BY_HANDLE_FILE_INFORMATION>(const BY_HANDLE_FILE_INFORMATION* info)
{
	return info->nNumberOfLinks;
}

template <typename INFO>
CHAOS_INLINE file_status status_from_INFO(const path& p, const INFO* info, std::error_code&, uintmax_t* sz = nullptr, time_t* lwt = nullptr) noexcept
{
	file_type ft = file_type::unknown;
...
CHAOS_INLINE bool is_not_found_error(std::error_code& ec)
{
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE file_status symlink_status_ex(const path& p, std::error_code& ec, uintmax_t* sz = nullptr, uintmax_t* nhl = nullptr, time_t* lwt = nullptr) noexcept
{
#ifdef CHAOS_OS_WINDOWS
	file_status fs;
...
CHAOS_INLINE file_status status_ex(const path& p, std::error_code& ec, file_status* sls = nullptr, uintmax_t* sz = nullptr, uintmax_t* nhl = nullptr, time_t* lwt = nullptr, int recurse_count = 0) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
	if (recurse_count > 16) {
...
CHAOS_INLINE u8arguments::u8arguments(int& argc, char**& argv)
	: _argc(argc)
	, _argv(argv)
	, _refargc(argc)
	, _refargv(argv)
...
CHAOS_INLINE path::path() noexcept {}

CHAOS_INLINE path::path(const path& p)
	: _path(p._path)
{
}

CHAOS_INLINE path::path(path&& p) noexcept
	: _path(std::move(p._path))
{
}

CHAOS_INLINE path::path(string_type&& source, format fmt)
#ifdef CHAOS_USE_WCHAR_T
	: _path(detail::toUtf8(source))
#else
...
CHAOS_INLINE path::~path() {}

//-----------------------------------------------------------------------------
// 30.10.8.4.2 assignments

CHAOS_INLINE path& path::operator=(const path& p)
{
	_path = p._path;
	return *this;
}

CHAOS_INLINE path& path::operator=(path&& p) noexcept
{
	_path = std::move(p._path);
	return *this;
}

CHAOS_INLINE path& path::operator=(path::string_type&& source)
{
	return assign(source);
}

CHAOS_INLINE path& path::assign(path::string_type&& source)
{
#ifdef CHAOS_USE_WCHAR_T
	_path = detail::toUtf8(source);
#else
...
CHAOS_INLINE void path::append_name(const char* name)
{
	if (_path.empty()) {
...
CHAOS_INLINE path& path::operator+=(const path& x)
{
	return concat(x._path);
}

CHAOS_INLINE path& path::operator+=(const string_type& x)
{
	return concat(x);
}

#ifdef __cpp_lib_string_view
CHAOS_INLINE path& path::operator+=(std::basic_string_view<value_type> x)
{
	return concat(x);
}
#endif

CHAOS_INLINE path& path::operator+=(const value_type* x)
{
	return concat(string_type(x));
}

CHAOS_INLINE path& path::operator+=(value_type x)
{
#ifdef CHAOS_OS_WINDOWS
	if (x == '\\') {
		x = generic_separator;
...
//-----------------------------------------------------------------------------
// 30.10.8.4.5 modifiers
CHAOS_INLINE void path::clear() noexcept
{
	_path.clear();
}

CHAOS_INLINE path& path::make_preferred()
{
...
CHAOS_INLINE path& path::remove_filename()
{
	if (has_filename()) {
...
CHAOS_INLINE path& path::replace_filename(const path& replacement)
{
	remove_filename();
	return append(replacement);
}

CHAOS_INLINE path& path::replace_extension(const path& replacement)
{
	if (has_extension()) {
...
CHAOS_INLINE void path::swap(path& rhs) noexcept
{
...
// 30.10.8.4.6, native format observers
#ifdef CHAOS_OS_WINDOWS
CHAOS_INLINE path::impl_string_type path::native_impl() const
{
...
#else
CHAOS_INLINE const path::impl_string_type& path::native_impl() const
{
	return _path;
}
#endif

CHAOS_INLINE const path::string_type& path::native() const
{
#ifdef CHAOS_OS_WINDOWS
#ifdef CHAOS_USE_WCHAR_T
...
CHAOS_INLINE const path::value_type* path::c_str() const
{
	return native().c_str();
}

CHAOS_INLINE path::operator path::string_type() const
{
	return native();
}
...
CHAOS_INLINE std::string path::string() const
{
	return native_impl();
}

CHAOS_INLINE std::wstring path::wstring() const
{
#ifdef CHAOS_USE_WCHAR_T
	return native();
#else
...
CHAOS_INLINE std::string path::u8string() const
{
	return native_impl();
}

CHAOS_INLINE std::u16string path::u16string() const
{
	return detail::fromUtf8<std::u16string>(native_impl());
}

CHAOS_INLINE std::u32string path::u32string() const
{
...
CHAOS_INLINE const std::string& path::generic_string() const
{
	return _path;
}

CHAOS_INLINE std::wstring path::generic_wstring() const
{
	return detail::fromUtf8<std::wstring>(_path);
}

CHAOS_INLINE std::string path::generic_u8string() const
{
	return _path;
}

CHAOS_INLINE std::u16string path::generic_u16string() const
{
	return detail::fromUtf8<std::u16string>(_path);
}

CHAOS_INLINE std::u32string path::generic_u32string() const
{
...
//-----------------------------------------------------------------------------
// 30.10.8.4.8, compare
CHAOS_INLINE int path::compare(const path& p) const noexcept
{
	return native().compare(p.native());
}

CHAOS_INLINE int path::compare(const string_type& s) const
{
	return native().compare(path(s).native());
}

#ifdef __cpp_lib_string_view
CHAOS_INLINE int path::compare(std::basic_string_view<value_type> s) const
{
	return native().compare(path(s).native());
}
#endif

CHAOS_INLINE int path::compare(const value_type* s) const
{
	return native().compare(path(s).native());
}
...
//-----------------------------------------------------------------------------
// 30.10.8.4.9, decomposition
CHAOS_INLINE path path::root_name() const
{
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE path path::root_directory() const
{
	path root = root_name();
...
CHAOS_INLINE path path::filename() const
{
	return relative_path().empty() ? path() : path(*--end());
}

CHAOS_INLINE path path::stem() const
{
...
CHAOS_INLINE path path::extension() const
{
	impl_string_type fn = filename().string();
...
//-----------------------------------------------------------------------------
// 30.10.8.4.10, query
CHAOS_INLINE bool path::empty() const noexcept
{
	return _path.empty();
}

CHAOS_INLINE bool path::has_root_name() const
{
	return !root_name().empty();
}

CHAOS_INLINE bool path::has_root_directory() const
{
	return !root_directory().empty();
}

CHAOS_INLINE bool path::has_root_path() const
{
	return !root_path().empty();
}

CHAOS_INLINE bool path::has_relative_path() const
{
	return !relative_path().empty();
}

CHAOS_INLINE bool path::has_parent_path() const
{
	return !parent_path().empty();
}

CHAOS_INLINE bool path::has_filename() const
{
	return !filename().empty();
}

CHAOS_INLINE bool path::has_stem() const
{
	return !stem().empty();
}

CHAOS_INLINE bool path::has_extension() const
{
	return !extension().empty();
}

CHAOS_INLINE bool path::is_absolute() const
{
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE bool path::is_relative() const
{
	return !is_absolute();
}
...
//-----------------------------------------------------------------------------
// 30.10.8.4.11, generation
CHAOS_INLINE path path::lexically_normal() const
{
	path dest;
...
CHAOS_INLINE path path::lexically_relative(const path& base) const
{
...
CHAOS_INLINE path path::lexically_proximate(const path& base) const
{
...
//-----------------------------------------------------------------------------
// 30.10.8.5, iterators
CHAOS_INLINE path::iterator::iterator() {}

CHAOS_INLINE path::iterator::iterator(const path::impl_string_type::const_iterator& first, const path::impl_string_type::const_iterator& last, const path::impl_string_type::const_iterator& pos)
	: _first(first)
	, _last(last)
	, _iter(pos)
{
...
CHAOS_INLINE path::impl_string_type::const_iterator path::iterator::increment(const path::impl_string_type::const_iterator& pos) const
{
...
CHAOS_INLINE path::impl_string_type::const_iterator path::iterator::decrement(const path::impl_string_type::const_iterator& pos) const
{
	path::impl_string_type::const_iterator i = pos;
	if (i != _first) {
		--i;
...
CHAOS_INLINE void path::iterator::updateCurrent()
{
...
CHAOS_INLINE path::iterator& path::iterator::operator++()
{
...
CHAOS_INLINE path::iterator path::iterator::operator++(int)
{
...
CHAOS_INLINE bool path::iterator::operator==(const path::iterator& other) const
{
...
CHAOS_INLINE path::iterator::reference path::iterator::operator*() const
{
	return _current;
}

CHAOS_INLINE path::iterator::pointer path::iterator::operator->() const
{
	return &_current;
}

CHAOS_INLINE path::iterator path::begin() const
{
	return iterator(_path.begin(), _path.end(), _path.begin());
}

CHAOS_INLINE path::iterator path::end() const
{
...
//-----------------------------------------------------------------------------
// 30.10.8.6, path non-member functions
CHAOS_INLINE void swap(path& lhs, path& rhs) noexcept
{
	swap(lhs._path, rhs._path);
}

CHAOS_INLINE size_t hash_value(const path& p) noexcept
{
	return std::hash<std::string>()(p.generic_string());
}

CHAOS_INLINE bool operator==(const path& lhs, const path& rhs) noexcept
{
...
CHAOS_INLINE path operator/(const path& lhs, const path& rhs)
{
	path result(lhs);
	result /= rhs;
	return result;
...
//-----------------------------------------------------------------------------
// 30.10.9 Class filesystem_error
CHAOS_INLINE filesystem_error::filesystem_error(const std::string& what_arg, std::error_code ec)
...
CHAOS_INLINE filesystem_error::filesystem_error(const std::string& what_arg, const path& p1, std::error_code ec)
	: std::system_error(ec, what_arg)
	, _what_arg(what_arg)
	, _ec(ec)
	, _p1(p1)
...
CHAOS_INLINE filesystem_error::filesystem_error(const std::string& what_arg, const path& p1, const path& p2, std::error_code ec)
	: std::system_error(ec, what_arg)
	, _what_arg(what_arg)
	, _ec(ec)
	, _p1(p1)
...
CHAOS_INLINE const path& filesystem_error::path1() const noexcept
{
	return _p1;
}

CHAOS_INLINE const path& filesystem_error::path2() const noexcept
{
	return _p2;
}

CHAOS_INLINE const char* filesystem_error::what() const noexcept
{
...
//-----------------------------------------------------------------------------
// 30.10.15, filesystem operations
CHAOS_INLINE path absolute(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE path absolute(const path& p, std::error_code& ec)
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
	if (p.empty()) {
...
CHAOS_INLINE path canonical(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE path canonical(const path& p, std::error_code& ec)
{
	if (p.empty()) {
...
CHAOS_INLINE void copy(const path& from, const path& to)
{
	copy(from, to, copy_options::none);
}

CHAOS_INLINE void copy(const path& from, const path& to, std::error_code& ec) noexcept
{
	copy(from, to, copy_options::none, ec);
}

CHAOS_INLINE void copy(const path& from, const path& to, copy_options options)
{
	std::error_code ec;
...
CHAOS_INLINE void copy(const path& from, const path& to, copy_options options, std::error_code& ec) noexcept
{
	std::error_code tec;
...
CHAOS_INLINE bool copy_file(const path& from, const path& to)
{
	return copy_file(from, to, copy_options::none);
}

CHAOS_INLINE bool copy_file(const path& from, const path& to, std::error_code& ec) noexcept
{
	return copy_file(from, to, copy_options::none, ec);
}

CHAOS_INLINE bool copy_file(const path& from, const path& to, copy_options option)
{
	std::error_code ec;
...
CHAOS_INLINE bool copy_file(const path& from, const path& to, copy_options options, std::error_code& ec) noexcept
{
...
CHAOS_INLINE void copy_symlink(const path& existing_symlink, const path& new_symlink)
{
	std::error_code ec;
...
CHAOS_INLINE void copy_symlink(const path& existing_symlink, const path& new_symlink, std::error_code& ec) noexcept
{
	ec.clear();
...
CHAOS_INLINE bool create_directories(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE bool create_directories(const path& p, std::error_code& ec) noexcept
{
	path current;
	ec.clear();
...
CHAOS_INLINE bool create_directory(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE bool create_directory(const path& p, std::error_code& ec) noexcept
{
	return create_directory(p, path(), ec);
}

CHAOS_INLINE bool create_directory(const path& p, const path& attributes)
{
	std::error_code ec;
...
CHAOS_INLINE bool create_directory(const path& p, const path& attributes, std::error_code& ec) noexcept
{
	std::error_code tec;
	ec.clear();
...
CHAOS_INLINE void create_directory_symlink(const path& to, const path& new_symlink)
{
	std::error_code ec;
	create_directory_symlink(to, new_symlink, ec);
	if (ec) {
...
CHAOS_INLINE void create_directory_symlink(const path& to, const path& new_symlink, std::error_code& ec) noexcept
{
	detail::create_symlink(to, new_symlink, true, ec);
}

CHAOS_INLINE void create_hard_link(const path& to, const path& new_hard_link)
{
	std::error_code ec;
...
CHAOS_INLINE void create_hard_link(const path& to, const path& new_hard_link, std::error_code& ec) noexcept
{
	detail::create_hardlink(to, new_hard_link, ec);
}

CHAOS_INLINE void create_symlink(const path& to, const path& new_symlink)
{
	std::error_code ec;
	create_symlink(to, new_symlink, ec);
	if (ec) {
...
CHAOS_INLINE void create_symlink(const path& to, const path& new_symlink, std::error_code& ec) noexcept
{
	detail::create_symlink(to, new_symlink, false, ec);
}

CHAOS_INLINE path current_path()
{
	std::error_code ec;
...
CHAOS_INLINE path current_path(std::error_code& ec)
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE void current_path(const path& p)
{
	std::error_code ec;
	current_path(p, ec);
	if (ec) {
...
CHAOS_INLINE void current_path(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE bool exists(file_status s) noexcept
{
	return status_known(s) && s.type() != file_type::not_found;
}

CHAOS_INLINE bool exists(const path& p)
{
	return exists(status(p));
}

CHAOS_INLINE bool exists(const path& p, std::error_code& ec) noexcept
{
...
CHAOS_INLINE bool equivalent(const path& p1, const path& p2)
{
	std::error_code ec;
...
CHAOS_INLINE bool equivalent(const path& p1, const path& p2, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE uintmax_t file_size(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE uintmax_t file_size(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
	WIN32_FILE_ATTRIBUTE_DATA attr;
...
CHAOS_INLINE uintmax_t hard_link_count(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE uintmax_t hard_link_count(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE bool is_block_file(file_status s) noexcept
{
	return s.type() == file_type::block;
}

CHAOS_INLINE bool is_block_file(const path& p)
{
	return is_block_file(status(p));
}

CHAOS_INLINE bool is_block_file(const path& p, std::error_code& ec) noexcept
{
	return is_block_file(status(p, ec));
}

CHAOS_INLINE bool is_character_file(file_status s) noexcept
{
	return s.type() == file_type::character;
}

CHAOS_INLINE bool is_character_file(const path& p)
{
	return is_character_file(status(p));
}

CHAOS_INLINE bool is_character_file(const path& p, std::error_code& ec) noexcept
{
	return is_character_file(status(p, ec));
}

CHAOS_INLINE bool is_directory(file_status s) noexcept
{
	return s.type() == file_type::directory;
}

CHAOS_INLINE bool is_directory(const path& p)
{
	return is_directory(status(p));
}

CHAOS_INLINE bool is_directory(const path& p, std::error_code& ec) noexcept
{
	return is_directory(status(p, ec));
}

CHAOS_INLINE bool is_empty(const path& p)
{
	if (is_directory(p)) {
...
CHAOS_INLINE bool is_empty(const path& p, std::error_code& ec) noexcept
{
	auto fs = status(p, ec);
	if (ec) {
		return false;
...
CHAOS_INLINE bool is_fifo(file_status s) noexcept
{
	return s.type() == file_type::fifo;
}

CHAOS_INLINE bool is_fifo(const path& p)
{
	return is_fifo(status(p));
}

CHAOS_INLINE bool is_fifo(const path& p, std::error_code& ec) noexcept
{
	return is_fifo(status(p, ec));
}

CHAOS_INLINE bool is_other(file_status s) noexcept
{
	return exists(s) && !is_regular_file(s) && !is_directory(s) && !is_symlink(s);
}

CHAOS_INLINE bool is_other(const path& p)
{
	return is_other(status(p));
}

CHAOS_INLINE bool is_other(const path& p, std::error_code& ec) noexcept
{
	return is_other(status(p, ec));
}

CHAOS_INLINE bool is_regular_file(file_status s) noexcept
{
	return s.type() == file_type::regular;
}

CHAOS_INLINE bool is_regular_file(const path& p)
{
	return is_regular_file(status(p));
}

CHAOS_INLINE bool is_regular_file(const path& p, std::error_code& ec) noexcept
{
	return is_regular_file(status(p, ec));
}

CHAOS_INLINE bool is_socket(file_status s) noexcept
{
	return s.type() == file_type::socket;
}

CHAOS_INLINE bool is_socket(const path& p)
{
	return is_socket(status(p));
}

CHAOS_INLINE bool is_socket(const path& p, std::error_code& ec) noexcept
{
	return is_socket(status(p, ec));
}

CHAOS_INLINE bool is_symlink(file_status s) noexcept
{
	return s.type() == file_type::symlink;
}

CHAOS_INLINE bool is_symlink(const path& p)
{
	return is_symlink(symlink_status(p));
}

CHAOS_INLINE bool is_symlink(const path& p, std::error_code& ec) noexcept
{
	return is_symlink(symlink_status(p, ec));
}

CHAOS_INLINE file_time_type last_write_time(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE file_time_type last_write_time(const path& p, std::error_code& ec) noexcept
{
	time_t result = 0;
	ec.clear();
...
CHAOS_INLINE void last_write_time(const path& p, file_time_type new_time)
{
	std::error_code ec;
...
CHAOS_INLINE void last_write_time(const path& p, file_time_type new_time, std::error_code& ec) noexcept
{
	ec.clear();
...
CHAOS_INLINE void permissions(const path& p, perms prms, perm_options opts)
{
	std::error_code ec;
...
CHAOS_INLINE void permissions(const path& p, perms prms, std::error_code& ec) noexcept
{
	permissions(p, prms, perm_options::replace, ec);
}

CHAOS_INLINE void permissions(const path& p, perms prms, perm_options opts, std::error_code& ec)
{
...
CHAOS_INLINE path proximate(const path& p, std::error_code& ec)
{
	return proximate(p, current_path(), ec);
}

CHAOS_INLINE path proximate(const path& p, const path& base)
{
	return weakly_canonical(p).lexically_proximate(weakly_canonical(base));
}

CHAOS_INLINE path proximate(const path& p, const path& base, std::error_code& ec)
{
	return weakly_canonical(p, ec).lexically_proximate(weakly_canonical(base, ec));
}

CHAOS_INLINE path read_symlink(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE path read_symlink(const path& p, std::error_code& ec)
{
...
CHAOS_INLINE path relative(const path& p, std::error_code& ec)
{
	return relative(p, current_path(ec), ec);
}

CHAOS_INLINE path relative(const path& p, const path& base)
{
	return weakly_canonical(p).lexically_relative(weakly_canonical(base));
}

CHAOS_INLINE path relative(const path& p, const path& base, std::error_code& ec)
{
	return weakly_canonical(p, ec).lexically_relative(weakly_canonical(base, ec));
}

CHAOS_INLINE bool remove(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE bool remove(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE uintmax_t remove_all(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE uintmax_t remove_all(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
	uintmax_t count = 0;
	if (p == "/") {
...
CHAOS_INLINE void rename(const path& from, const path& to)
{
	std::error_code ec;
	rename(from, to, ec);
	if (ec) {
...
CHAOS_INLINE void rename(const path& from, const path& to, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
	if (from != to) {
...
CHAOS_INLINE void resize_file(const path& p, uintmax_t size)
{
	std::error_code ec;
...
CHAOS_INLINE void resize_file(const path& p, uintmax_t size, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE space_info space(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE space_info space(const path& p, std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE file_status status(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE file_status status(const path& p, std::error_code& ec) noexcept
{
	return detail::status_ex(p, ec);
}

CHAOS_INLINE bool status_known(file_status s) noexcept
{
	return s.type() != file_type::none;
}

CHAOS_INLINE file_status symlink_status(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE file_status symlink_status(const path& p, std::error_code& ec) noexcept
{
	return detail::symlink_status_ex(p, ec);
}

CHAOS_INLINE path temp_directory_path()
{
	std::error_code ec;
...
CHAOS_INLINE path temp_directory_path(std::error_code& ec) noexcept
{
	ec.clear();
#ifdef CHAOS_OS_WINDOWS
	wchar_t buffer[512];
...
CHAOS_INLINE path weakly_canonical(const path& p)
{
	std::error_code ec;
...
CHAOS_INLINE path weakly_canonical(const path& p, std::error_code& ec) noexcept
{
	path result;
	ec.clear();
	bool scan = true;
...
// 30.10.11 class file_status
// 30.10.11.1 constructors and destructor
CHAOS_INLINE file_status::file_status() noexcept
	: file_status(file_type::none)
{
}

CHAOS_INLINE file_status::file_status(file_type ft, perms prms) noexcept
	: _type(ft)
	, _perms(prms)
{
}

CHAOS_INLINE file_status::file_status(const file_status& other) noexcept
	: _type(other._type)
	, _perms(other._perms)
{
}

CHAOS_INLINE file_status::file_status(file_status&& other) noexcept
	: _type(other._type)
	, _perms(other._perms)
{
}

CHAOS_INLINE file_status::~file_status() {}

// assignments:
CHAOS_INLINE file_status& file_status::operator=(const file_status& rhs) noexcept
{
	_type = rhs._type;
	_perms = rhs._perms;
	return *this;
...
CHAOS_INLINE file_status& file_status::operator=(file_status&& rhs) noexcept
{
	_type = rhs._type;
	_perms = rhs._perms;
	return *this;
...
// 30.10.11.3 modifiers
CHAOS_INLINE void file_status::type(file_type ft) noexcept
{
	_type = ft;
}

CHAOS_INLINE void file_status::permissions(perms prms) noexcept
{
	_perms = prms;
}

// 30.10.11.2 observers
CHAOS_INLINE file_type file_status::type() const noexcept
{
	return _type;
}

CHAOS_INLINE perms file_status::permissions() const noexcept
{
	return _perms;
}
...
CHAOS_INLINE directory_entry::directory_entry(const filesystem::path& p)
	: _path(p)
	, _file_size(0)
#ifndef CHAOS_OS_WINDOWS
	, _hard_link_count(0)
...
CHAOS_INLINE directory_entry::directory_entry(const filesystem::path& p, std::error_code& ec)
	: _path(p)
	, _file_size(0)
#ifndef CHAOS_OS_WINDOWS
	, _hard_link_count(0)
...
CHAOS_INLINE directory_entry::~directory_entry() {}
...
CHAOS_INLINE void directory_entry::assign(const filesystem::path& p)
{
	_path = p;
	refresh();
}

CHAOS_INLINE void directory_entry::assign(const filesystem::path& p, std::error_code& ec)
{
	_path = p;
	refresh(ec);
}

CHAOS_INLINE void directory_entry::replace_filename(const filesystem::path& p)
{
	_path.replace_filename(p);
	refresh();
}

CHAOS_INLINE void directory_entry::replace_filename(const filesystem::path& p, std::error_code& ec)
{
	_path.replace_filename(p);
	refresh(ec);
}

CHAOS_INLINE void directory_entry::refresh()
{
	std::error_code ec;
	refresh(ec);
	if (ec) {
...
CHAOS_INLINE void directory_entry::refresh(std::error_code& ec) noexcept
{
#ifdef CHAOS_OS_WINDOWS
...
CHAOS_INLINE const filesystem::path& directory_entry::path() const noexcept
{
	return _path;
}

CHAOS_INLINE directory_entry::operator const filesystem::path&() const noexcept
{
	return _path;
}

CHAOS_INLINE bool directory_entry::exists() const
{
	return filesystem::exists(status());
}

CHAOS_INLINE bool directory_entry::exists(std::error_code& ec) const noexcept
{
	return filesystem::exists(status(ec));
}

CHAOS_INLINE bool directory_entry::is_block_file() const
{
	return filesystem::is_block_file(status());
}
CHAOS_INLINE bool directory_entry::is_block_file(std::error_code& ec) const noexcept
{
	return filesystem::is_block_file(status(ec));
}

CHAOS_INLINE bool directory_entry::is_character_file() const
{
	return filesystem::is_character_file(status());
}

CHAOS_INLINE bool directory_entry::is_character_file(std::error_code& ec) const noexcept
{
	return filesystem::is_character_file(status(ec));
}

CHAOS_INLINE bool directory_entry::is_directory() const
{
	return filesystem::is_directory(status());
}

CHAOS_INLINE bool directory_entry::is_directory(std::error_code& ec) const noexcept
{
	return filesystem::is_directory(status(ec));
}

CHAOS_INLINE bool directory_entry::is_fifo() const
{
	return filesystem::is_fifo(status());
}

CHAOS_INLINE bool directory_entry::is_fifo(std::error_code& ec) const noexcept
{
	return filesystem::is_fifo(status(ec));
}

CHAOS_INLINE bool directory_entry::is_other() const
{
	return filesystem::is_other(status());
}

CHAOS_INLINE bool directory_entry::is_other(std::error_code& ec) const noexcept
{
	return filesystem::is_other(status(ec));
}

CHAOS_INLINE bool directory_entry::is_regular_file() const
{
	return filesystem::is_regular_file(status());
}

CHAOS_INLINE bool directory_entry::is_regular_file(std::error_code& ec) const noexcept
{
	return filesystem::is_regular_file(status(ec));
}

CHAOS_INLINE bool directory_entry::is_socket() const
{
	return filesystem::is_socket(status());
}

CHAOS_INLINE bool directory_entry::is_socket(std::error_code& ec) const noexcept
{
	return filesystem::is_socket(status(ec));
}

CHAOS_INLINE bool directory_entry::is_symlink() const
{
	return filesystem::is_symlink(symlink_status());
}

CHAOS_INLINE bool directory_entry::is_symlink(std::error_code& ec) const noexcept
{
	return filesystem::is_symlink(symlink_status(ec));
}

CHAOS_INLINE uintmax_t directory_entry::file_size() const
{
...
CHAOS_INLINE uintmax_t directory_entry::file_size(std::error_code& ec) const noexcept
{
	if (_status.type() != file_type::none) {
		return _file_size;
	}
...
CHAOS_INLINE uintmax_t directory_entry::hard_link_count() const
{
#ifndef CHAOS_OS_WINDOWS
	if (_status.type() != file_type::none) {
...
CHAOS_INLINE uintmax_t directory_entry::hard_link_count(std::error_code& ec) const noexcept
{
#ifndef CHAOS_OS_WINDOWS
	if (_status.type() != file_type::none) {
		return _hard_link_count;
...
CHAOS_INLINE file_time_type directory_entry::last_write_time() const
{
	if (_status.type() != file_type::none) {
...
CHAOS_INLINE file_time_type directory_entry::last_write_time(std::error_code& ec) const noexcept
{
	if (_status.type() != file_type::none) {
		return std::chrono::system_clock::from_time_t(_last_write_time);
	}
...
CHAOS_INLINE file_status directory_entry::status() const
{
	if (_status.type() != file_type::none) {
		return _status;
	}
...
CHAOS_INLINE file_status directory_entry::status(std::error_code& ec) const noexcept
{
	if (_status.type() != file_type::none) {
		return _status;
	}
...
CHAOS_INLINE file_status directory_entry::symlink_status() const
{
...
CHAOS_INLINE file_status directory_entry::symlink_status(std::error_code& ec) const noexcept
{
	if (_symlink_status.type() != file_type::none) {
		return _symlink_status;
	}
...
CHAOS_INLINE bool directory_entry::operator<(const directory_entry& rhs) const noexcept
{
...
// 30.10.13.1 member functions
CHAOS_INLINE directory_iterator::directory_iterator() noexcept
	: _impl(new impl(path(), directory_options::none))
{
}

CHAOS_INLINE directory_iterator::directory_iterator(const path& p)
...
CHAOS_INLINE directory_iterator::directory_iterator(const path& p, directory_options options)
...
CHAOS_INLINE directory_iterator::directory_iterator(const path& p, std::error_code& ec) noexcept
	: _impl(new impl(p, directory_options::none))
{
	if (_impl->_ec) {
		ec = _impl->_ec;
...
CHAOS_INLINE directory_iterator::directory_iterator(const path& p, directory_options options, std::error_code& ec) noexcept
	: _impl(new impl(p, options))
{
	if (_impl->_ec) {
		ec = _impl->_ec;
...
CHAOS_INLINE directory_iterator::directory_iterator(const directory_iterator& rhs)
	: _impl(rhs._impl)
{
}

CHAOS_INLINE directory_iterator::directory_iterator(directory_iterator&& rhs) noexcept
	: _impl(std::move(rhs._impl))
{
}

CHAOS_INLINE directory_iterator::~directory_iterator() {}

CHAOS_INLINE directory_iterator& directory_iterator::operator=(const directory_iterator& rhs)
{
	_impl = rhs._impl;
	return *this;
}

CHAOS_INLINE directory_iterator& directory_iterator::operator=(directory_iterator&& rhs) noexcept
{
	_impl = std::move(rhs._impl);
	return *this;
}

CHAOS_INLINE const directory_entry& directory_iterator::operator*() const
{
...
CHAOS_INLINE directory_iterator& directory_iterator::operator++()
{
	std::error_code ec;
	_impl->increment(ec);
	if (ec) {
...
CHAOS_INLINE directory_iterator& directory_iterator::increment(std::error_code& ec) noexcept
{
	_impl->increment(ec);
	return *this;
}

CHAOS_INLINE bool directory_iterator::operator==(const directory_iterator& rhs) const
{
...
CHAOS_INLINE directory_iterator begin(directory_iterator iter) noexcept
{
	return iter;
}

CHAOS_INLINE directory_iterator end(const directory_iterator&) noexcept
{
...
CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator() noexcept
	: _impl(new recursive_directory_iterator_impl(directory_options::none, true))
{
	_impl->_dir_iter_stack.push(directory_iterator());
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(const path& p)
	: _impl(new recursive_directory_iterator_impl(directory_options::none, true))
{
	_impl->_dir_iter_stack.push(directory_iterator(p));
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(const path& p, directory_options options)
	: _impl(new recursive_directory_iterator_impl(options, true))
{
	_impl->_dir_iter_stack.push(directory_iterator(p, options));
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(const path& p, directory_options options, std::error_code& ec) noexcept
	: _impl(new recursive_directory_iterator_impl(options, true))
{
	_impl->_dir_iter_stack.push(directory_iterator(p, options, ec));
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(const path& p, std::error_code& ec) noexcept
	: _impl(new recursive_directory_iterator_impl(directory_options::none, true))
{
	_impl->_dir_iter_stack.push(directory_iterator(p, ec));
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(const recursive_directory_iterator& rhs)
	: _impl(rhs._impl)
{
}

CHAOS_INLINE recursive_directory_iterator::recursive_directory_iterator(recursive_directory_iterator&& rhs) noexcept
	: _impl(std::move(rhs._impl))
{
}

CHAOS_INLINE recursive_directory_iterator::~recursive_directory_iterator() {}

// 30.10.14.1 observers
CHAOS_INLINE directory_options recursive_directory_iterator::options() const
{
	return _impl->_options;
}

CHAOS_INLINE int recursive_directory_iterator::depth() const
{
	return static_cast<int>(_impl->_dir_iter_stack.size() - 1);
}

CHAOS_INLINE bool recursive_directory_iterator::recursion_pending() const
{
	return _impl->_recursion_pending;
}

CHAOS_INLINE const directory_entry& recursive_directory_iterator::operator*() const
{
...
// 30.10.14.1 modifiers recursive_directory_iterator&
CHAOS_INLINE recursive_directory_iterator& recursive_directory_iterator::operator=(const recursive_directory_iterator& rhs)
{
	_impl = rhs._impl;
	return *this;
}

CHAOS_INLINE recursive_directory_iterator& recursive_directory_iterator::operator=(recursive_directory_iterator&& rhs) noexcept
{
	_impl = std::move(rhs._impl);
	return *this;
}

CHAOS_INLINE recursive_directory_iterator& recursive_directory_iterator::operator++()
{
	std::error_code ec;
	increment(ec);
	if (ec) {
...
CHAOS_INLINE recursive_directory_iterator& recursive_directory_iterator::increment(std::error_code& ec) noexcept
{
...
CHAOS_INLINE void recursive_directory_iterator::pop()
{
	std::error_code ec;
	pop(ec);
	if (ec) {
...
CHAOS_INLINE void recursive_directory_iterator::pop(std::error_code& ec)
{
	if (depth() == 0) {
...
CHAOS_INLINE void recursive_directory_iterator::disable_recursion_pending()
{
	_impl->_recursion_pending = false;
}

// other members as required by 27.2.3, input iterators
CHAOS_INLINE bool recursive_directory_iterator::operator==(const recursive_directory_iterator& rhs) const
{
...
// 30.10.14.2 directory_iterator non-member functions
CHAOS_INLINE recursive_directory_iterator begin(recursive_directory_iterator iter) noexcept
{
	return iter;
}

CHAOS_INLINE recursive_directory_iterator end(const recursive_directory_iterator&) noexcept
{
